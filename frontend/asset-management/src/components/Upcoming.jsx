...
const UpcomingAssets = () => {
  ...
  const fetchAssets = async () => {
    try {
      const response = await fetch("https://asset-management-1-0mj8.onrender.com/upcomingassets");
  ...
  const handleMoveToWarehouse = async (asset) => {
    try {
      ...
      const response = await fetch("https://asset-management-1-0mj8.onrender.com/warehouse", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      await fetch(`https://asset-management-1-0mj8.onrender.com/upcomingassets/${asset._id}`, {
        method: "DELETE",
      });
